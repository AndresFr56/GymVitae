...
public class UpdatePersonal extends JPanel {
  ...
  public boolean validateForm() {
    ...
    if (txtNombres.getText().trim().isEmpty()
      ...
      showErrorMessage("Por favor rellenar todos los campos");
      return false;
    }

    // Validar longitud de cédula
    if (txtCedula.getText().trim().length() != 10) {
      showErrorMessage("La cédula debe tener exactamente 10 dígitos");
      return false;
    }

    // Validar longitud de teléfono
    if (txtTelefono.getText().trim().length() != 10) {
      showErrorMessage("El teléfono debe tener exactamente 10 dígitos");
      return false;
    }
  ...
  public boolean updateEmpleado() {
    ...
    try {
      ...
      String message = "Error al actualizar empleado: " + e.getMessage();
      LOGGER.log(Level.SEVERE, message, e);
      showErrorMessage("Error inesperado al actualizar el empleado");
      return false;
    }
  }
