...
public class RegisterMembresia extends JPanel {
  ...
  private void initializeComponents() {
    ...
    // Cargar tipos de membresía activos
    ...
    // Listener para calcular automáticamente la fecha fin
    ...
    dateFechaFin.setEnabled(false); // Calculado automáticamente
  ...
  private void applyStyles() {
    ...
    txtFacturaId.putClientProperty(FlatClientProperties.PLACEHOLDER_TEXT, "ID de factura");
  ...
  private void buildForm() {
    ...
    contentPanel.add(new JLabel("ID Factura *"));
    contentPanel.add(txtFacturaId);
  ...
  public boolean validateForm() {
    ...
    try {
      ...
      if (txtFacturaId.getText().trim().isEmpty()) {
        showErrorMessage("El ID de factura es obligatorio");
        txtFacturaId.requestFocus();
        return false;
      }

      try {
        Integer.parseInt(txtFacturaId.getText().trim());
      } catch (NumberFormatException e) {
        showErrorMessage("El ID de factura debe ser un número válido");
        txtFacturaId.requestFocus();
        return false;
      }
