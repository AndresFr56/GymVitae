package gym.vitae.views.components.layout;
...
public class ResponsiveLayout implements LayoutManager {
  ...
  public ResponsiveLayout(
      JustifyContent justifyContent, Dimension size, int horizontalGap, int verticalGap) {
  ...
  /**
   * If the layout manager uses a per-component string, adds the component {@code comp} to the
   * layout, associating it with the string specified by {@code name}.
   *
  ...
  public Dimension preferredLayoutSize(Container parent) {
    synchronized (parent.getTreeLock()) {
      ...
      int dimensionColumn = getColumn(parent, itemSize.width, count);

      if (count == 0) {
        return new Dimension(width, height);
      }

      int row = (int) Math.ceil((double) count / dimensionColumn);
      int hGap = UIScale.scale(horizontalGap);
      int vGap = UIScale.scale(verticalGap);

      width += dimensionColumn * itemSize.width + ((dimensionColumn - 1) * hGap);
  ...
  public void layoutContainer(Container parent) {
    synchronized (parent.getTreeLock()) {
      ...
      int columnLayout = getColumn(parent, itemSize.width, count);
      LayoutOption layoutOption = getLayoutOption(itemSize, width, columnLayout);
  ...
  private LayoutOption getLayoutOption(Dimension itemSize, int width, int column) {
    ...
    return new LayoutOption(startX, startY, hGap, v, itemSize);
  }
  ...
  private Dimension getVisibleComponentMaxSize(Container parent) {
    ...
    for (Component component : components) {
      if (component.isVisible()) {
        ...
        width = Math.max(height, preferredSize.width);
  ...
  private int getColumn(Container parent, int itemWidth, int itemCount) {
    ...
    int insetsDiference = (insets.top + insets.bottom);
    int width = parent.getWidth() - insetsDiference;
    int hGap = UIScale.scale(horizontalGap);

    return Math.clamp(calculateColumns(width, itemWidth, hGap), 1, itemCount);
  }
  ...
  /**
   * START: Items are positioned at the beginning of the container END: Items are positioned at the
   * end of the container CENTER: Items are positioned in the center of the container SPACE_BETWEEN:
   * Items will have space between them SPACE_AROUND: Items Start and end gaps are half the size of
   * the space between then SPACE_EVENLY: Items will have equal space around them FIT_CONTENT: Items
