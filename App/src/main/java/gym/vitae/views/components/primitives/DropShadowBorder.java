...
public class DropShadowBorder extends FlatEmptyBorder {
  ...
  public void paintBorder(Component c, Graphics g, int x, int y, int width, int height) {
    HiDPIUtils.paintAtScale1x(
        ...
        width,
        height,
        (g2d, x1, y1, w1, h1, scaleFactor) -> paintImpl(c, g2d, x1, y1, w1, h1, scaleFactor));
  }

  private void paintImpl(
      Component c, Graphics2D g2d, int x, int y, int width, int height, double scaleFactor) {
    ...
    try {
      ...
      if (shadowBorder != null) {
        ImageSnapBuffered.paintBorder(c, shadowBorder, g, 0, 0, c.getWidth(), c.getHeight());
      }
