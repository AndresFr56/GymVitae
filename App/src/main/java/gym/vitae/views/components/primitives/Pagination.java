...
public class Pagination extends JPagination {

  private transient Animator animator;
  private float animate;

  private transient Rectangle2D fromRec;
  private transient Rectangle2D toRec;
  ...
  private void init() {
    ...
    setItemRenderer(
        new DefaultPaginationItemRenderer() {

          private int index;

          @Override
          public Component getPaginationItemRendererComponent(
              ...
              int index) {
            this.index = index;
            super.getPaginationItemRendererComponent(
                pagination, page, isSelected, isPressed, hasFocus, index);
            if (index == -2) {
              // for custom animation
              setContentAreaFilled(false);
            } else {
              setContentAreaFilled(true);
              FlatLafStyleUtils.appendStyle(
                  this,
                  "background:$Panel.background;" + "arc:18;" + "borderWidth:0;" + "focusWidth:0;");
            }
            return this;
          }
  ...
  public void paint(Graphics g) {
    ...
    if (animator.isRunning()) {
      ...
      rec =
          getRectangleOfIndex(((DefaultPaginationModel) getModel()).getIndexOf(getSelectedPage()));
    }
    if (rec != null) {
  ...
  private void paintSelected(Graphics g, Rectangle2D rec, float animate) {
    ...
    if (accentColor1 != null && accentColor2 != null) {
      g2.setPaint(
          new GradientPaint(
              0, 0, accentColor1, (float) rec.getWidth(), (float) rec.getHeight(), accentColor2));
    } else {
    ...
    Component newCom =
        getItemRenderer()
            .getPaginationItemRendererComponent(
                this, new Page(getSelectedPage(), Page.Type.PAGE), false, false, false, -2);
