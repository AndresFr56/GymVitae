...
public class CellsBorder implements Border {

  private final JList list;
  private final String title;

  private CellsBorder(JList list, String title) {
    this.list = list;
    this.title = title;
  }

  static CellsBorder createCellsBorder(JList list, String title) {
    return new CellsBorder(list, title);
  }
  ...
  public void paintBorder(Component c, Graphics g, int x, int y, int width, int height) {
    ...
    try {
      ...
      if (sepWidth > 0) {
        ...
        float xGapInner = (gap + x);
        float xGapEnd = (x + (width - gap - sepWidth));

        g2.fill(new Rectangle2D.Float(xGapInner, sy, sepWidth, sepHeight));
        g2.fill(new Rectangle2D.Float(xGapEnd, sy, sepWidth, sepHeight));
      }
