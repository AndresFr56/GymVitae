  ...
  public BeneficioDetalleDTO createBeneficio(BeneficioCreateDTO dto) {
    ...
    if (!dto.getNombre().matches("^[a-zA-Z\\s]+$")) {
      throw new IllegalArgumentException(
          "El nombre del beneficio solo debe contener letras y espacios.");
    }
  ...
  public BeneficioDetalleDTO updateBeneficio(int id, BeneficioUpdateDTO dto) {
    ...
    Beneficio beneficio =
        beneficioRepository
            .findById(id)
            .orElseThrow(
                () -> new IllegalArgumentException("Beneficio no encontrado con ID: " + id));
    ...
    return beneficioRepository
        .findDetalleById(id)
        .orElseThrow(
            () -> new IllegalStateException("Error al recuperar el beneficio actualizado"));
  }
